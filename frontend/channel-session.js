/**
 * T094: cancellable channel-analysis polling session.
 * Mirrors analysis-session: generation token + AbortController.
 * Terminal parent statuses: completed | degraded | failed.
 */
...
(function (global) {
  ...
  function createChannelSession(options) {
    options = options || {};
    var fetchImpl =
      options.fetch ||
        ...
        while (true) {
          ...
          try {
            var res = await fetchImpl(
              apiRoot + "/v1/channel-analyses/" + encodeURIComponent(taskId),
              { signal: signal }
            );
            if (!stillCurrent()) return { ok: false, cancelled: true };
            var body;
            ...
            // Terminal success for polling (caller inspects status/result).
            if (st === "completed" || st === "degraded" || st === "failed") {
              return { ok: true, body: body };
            }
            // Keep polling while queued / analyzing / other non-terminal.
  ...
  var api = {
    createChannelSession: createChannelSession,
    POLL_CAP_MS: POLL_CAP_MS,
    TRANSIENT_RETRY_LIMIT: TRANSIENT_RETRY_LIMIT,
    POLL_INTERVAL_MS: POLL_INTERVAL_MS,
  };
  global.createChannelSession = createChannelSession;
  global.ChannelSession = api;
